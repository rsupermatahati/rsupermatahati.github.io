...
const navbarClasses = computed(() => {
  ...
  if (!isScrolled.value) {
    return `${baseClasses} bg-emerald-600 dark:bg-neutral-900 border-b border-b-gray-200/50 dark:border-b-neutral-800`;
  } else {
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center h-16">
        ...
        <div class="flex items-center shrink-0">
          <router-link to="/" class="flex items-center transition-all duration-300 hover:scale-105 active:scale-100"
            @click="closeMobileMenu" aria-label="RSU Permata Hati Home">
            <img :src="currentLogo" alt="RSU Permata Hati" class="h-10 w-auto transition-opacity duration-300" />
          </router-link>
        </div>

        <!-- Desktop Navigation -->
        <div class="hidden md:flex md:items-center md:space-x-1">
          <div v-for="(menu, index) in menus" :key="menu.label" class="nav-dropdown relative">
            <!-- Menu without children -->
            <router-link v-if="!menu.children" :to="menu.path"
              class="nav-menu-item px-4 py-2 rounded-lg font-medium transition-all duration-200" :class="[
            ...
            <!-- Menu with children -->
            <button v-else @click="toggleMenu(index)"
              ...
              :class="[
                isChildActive(menu.children)
                  ? 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 shadow-sm'
                  : `${textColorClasses} ${hoverTextColorClasses} hover:bg-white/20 dark:hover:bg-neutral-800`,
                !isScrolled && !isDark ? 'hover:bg-white' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800'
              ]" :aria-expanded="openMenu === index" :aria-controls="`desktop-menu-${index}`">
              <span>{{ menu.label }}</span>
              <svg class="w-4 h-4 transition-transform duration-200 shrink-0"
                :class="{ 'rotate-180': openMenu === index }" fill="currentColor" viewBox="0 0 20 20"
                aria-hidden="true">
            ...
            <!-- Desktop Dropdown -->
            <div v-if="openMenu === index && menu.children" :id="`desktop-menu-${index}`"
              ...
              <ul class="py-2">
                <li v-for="child in menu.children" :key="child.label">
                  <router-link :to="child.path"
                    class="block px-4 py-3 text-sm transition-all duration-200 mx-2 rounded-lg" :class="{
          ...
          <button @click="toggleTheme" class="p-2 rounded-lg transition-all duration-200" :class="[
            textColorClasses,
            hoverTextColorClasses,
            !isScrolled && !isDark ? 'hover:bg-emerald-700' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800'
          ]" :title="currentThemeLabel" :aria-label="currentThemeLabel">
        ...
        <div class="flex items-center md:hidden space-x-2">
          ...
          <!-- Mobile Menu Toggle -->
          <button @click="isOpen = !isOpen" class="p-2 rounded-lg transition-all duration-200" :class="[
            textColorClasses,
            hoverTextColorClasses,
            !isScrolled && !isDark ? 'hover:bg-emerald-700' : 'hover:bg-neutral-100 dark:hover:bg-neutral-800'
          ]" :aria-expanded="isOpen" aria-label="Toggle navigation menu" aria-controls="mobile-menu">
            <svg class="h-6 w-6 transition-transform duration-200" :class="{ 'rotate-90': isOpen }" fill="none"
      ...
      <div v-if="isOpen" id="mobile-menu"
        ...
        <div class="px-4 py-3 space-y-1">
          <div v-for="(menu, index) in menus" :key="menu.label">
            <!-- Mobile Menu Item -->
            <div>
              <button v-if="menu.children" @click="toggleMobileMenu(index)"
                ...
                }" :aria-expanded="openMobileMenu === index" :aria-controls="`mobile-submenu-${index}`">
                <span>{{ menu.label }}</span>
                <svg class="w-4 h-4 transition-transform duration-200 shrink-0"
                  :class="{ 'rotate-180': openMobileMenu === index }" fill="currentColor" viewBox="0 0 20 20"
                  aria-hidden="true">
                  <path fill-rule="evenodd"
                    d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                    clip-rule="evenodd" />
              ...
              <router-link v-else :to="menu.path"
                class="block w-full text-left py-3 px-2 rounded-lg font-medium transition-all duration-200" :class="{
            ...
            <transition enter-active-class="transition-all duration-300 ease-out"
              ...
              enter-to-class="opacity-100 max-h-48" leave-from-class="opacity-100 max-h-48"
              leave-to-class="opacity-0 max-h-0">
              <div v-if="openMobileMenu === index && menu.children" :id="`mobile-submenu-${index}`"
                ...
                <router-link v-for="child in menu.children" :key="child.label" :to="child.path"
                  class="block py-2 px-3 rounded-lg text-sm transition-all duration-200" :class="{
                    'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30 font-semibold': isActiveRoute(child.path),
